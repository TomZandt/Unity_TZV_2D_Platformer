...
public class StateMachine_Action_Patrol : StateMachine_Action
{
    ...
    public override void Act(StateMachine_Controller _controller)
    {
        ...
        {
            _controller.transform.position = Vector2.MoveTowards(_controller.transform.position, _controller.waypoints[0].position, _controller.enemyStats.enemyMoveSpeed * Time.deltaTime);
        }
        else
        {
            isWaypoint1 = false;
            ...
            {
                _controller.transform.position = Vector2.MoveTowards(_controller.transform.position, _controller.waypoints[1].position, _controller.enemyStats.enemyMoveSpeed * Time.deltaTime);
            }
            else
            {
                isWaypoint1 = true;
