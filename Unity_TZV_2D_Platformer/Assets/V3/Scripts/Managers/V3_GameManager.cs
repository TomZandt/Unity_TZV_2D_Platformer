...
public class V3_GameManager : MonoBehaviour
{
    ...
    static V3_GameManager current;

    public float deathSequenceDuration = 1.5f;  //How long player death takes before restarting

    List<V3_Orb> orbs;                             //The collection of scene orbs
    V3_Door lockedDoor;                            //The scene door
    V3_SceneFader sceneFader;                      //The scene fader
    ...
    void Update()
    {
        ...
        V3_UIManager.UpdateTimeUI(totalGameTime);
    }
    ...
    public static void PlayerGrabbedOrb(V3_Orb orb)
    {
        //If there is no current Game Manager, exit
        if (current == null)
            return;
    ...
    void RestartScene()
    {
        ...
        V3_AudioManager.PlaySceneRestartAudio();
